...
CLASS_FIELDS = ['className', 'classType', 'duration', 'classCapacity', 'instructorId', 'gymId']
...
def updateClass(conn):
    ...
    if updateField == "instructorId" and not recordExists(conn, "Instructor", "instructorId", value):
        print("Invalid instructor ID")
        return
    ...
    curse = conn.cursor()
    curse.execute(f"UPDATE Class SET {updateField} = ? WHERE classId = ?", (value, classId))
    conn.commit()
    print("Class updated.")
    prompt_enter
    ...
    if attendeeCount > 0:
        ...
        if move == 'y':
            ...
            if not recordExists(conn, "Class", "classId", newClassId):
                ...
                return
            curse.execute("UPDATE Attends SET classId = ? WHERE classId = ?", (newClassId, classId))
        else:
